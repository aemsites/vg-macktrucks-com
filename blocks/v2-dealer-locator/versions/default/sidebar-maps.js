...
$.fn.renderPinDetails = async function (markerId) {
  ...
  if (markerDetails.WEB_ADDRESS) {
    ...
    templateClone.find('.detail-website a').css({ 'pointer-events': 'none', 'cursor': 'default', 'opacity': '0.5', 'border-color': 'red' });
...
$.fn.tmpPins = function (tmpPinList) {
  ...
  // Verify that the button with id loadMorePinsBtn doesn't already exist before creating it
  if ($('#loadMorePinsBtn').length === 0) {
    $("<button/>", {
      text: "View More",
  ...
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(function (position) {
      ...
      if ($map) {
        ...
        console.error('%cmap %cnot set', 'color: red;', 'color: default;', { $map });
      }
